...
The returned JSON is stored in `network_details` under key `ext.details` and
...
per-network `ext.details` blob is removed from CloudStack after a successful
return.
...
placeholder NIC IP (if any) and the `ext.details` blob are cleaned up
